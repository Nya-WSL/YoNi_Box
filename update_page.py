...
def page():
    with ui.row():
        ui.button("返回主页", on_click=lambda: ui.open('/'), color="#735798").classes("text-white")
        ui.button("GitHub", on_click=lambda: ui.open('https://github.com/Nya-WSL/YoNi_Box', new_tab=True), color="#735798").classes("text-white")
    with ui.timeline(side='right', layout='comfortable', color='red'):
        ui.timeline_entry('YoNi\'s SendBox Decoration Log', heading=True)
        ...
        ui.timeline_entry('登录界面新建返回按钮 | 登录界面和后台新增背景 | 优化当棉花糖为空时后台的显示效果', title='Release of 1.1.1', subtitle='2024-01-01')
        ...
        ui.timeline_entry('主页标题新增版本号 | 修改所有按钮的样式 | 部分组件颜色改为#735798 | 投稿页面新增返回主页按钮 | 现在投稿可以输入自定义昵称，留空默认为投稿时间 | 棉花糖页面新增可切换的紧凑模式，默认为关，该模式可能会缓解当某一条棉花糖有过多换行符时会导致背景Y轴被严重拉伸的问题 | 尝试通过将注意事项部分组件的值改为HTML元素来缓解部分设备的排版问题 | 修复因为显示棉花糖的组件的值是HTML元素导致的棉花糖中的换行符无效的问题 | 修复一个在服务器反代环境下头像文件路径错误的问题', title='Release of 1.2.0', subtitle='2024-01-05')
        ...
        ui.timeline_entry('优化数据库模块代码 | 重写备份模块，现在除了本地备份还会将数据库文件上传到七牛云', title='Release of 1.2.7', subtitle='2024-01-17')
        ui.timeline_entry('移除主页登记按钮的透明背景以适配移动端', title='Release of 1.2.7.2', subtitle='2024-05-02', avatar='static/YoNi.jpg')